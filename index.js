...
app.get("/groups", async (req, res) => {
    try {
        ...
        const groups = Object.values(chats).map(g => ({
            ...
            jid: g.id || "N/A",  // ✅ Use "jid" key instead of "id" for clarity
            participantsCount: g.participants ? g.participants.length : 0
        }));

        if (groups.length === 0) {
            console.log("⚠️ No groups found for this session.");
            return res.json({ message: "No groups found on this session." });
        }

        // ✅ Log and save the full group list
        ...
        groups.forEach(g => console.log(`➡️ ${g.name} (${g.jid})`));
